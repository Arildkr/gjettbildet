...
io.on('connection', (socket) => {
   ...
   * Mottar nå correctAnswerText for å sende fasit til studenter
   */
  socket.on('host:validate-answer', ({ roomCode, playerId, isCorrect, correctAnswerText }) => {
    ...
    if (result.correct) {
      // Riktig svar - Send fasit til alle
      io.to(roomCode).emit('game:answer-result', {
        playerId,
        isCorrect: true,
        points: result.points,
        correctAnswerText: correctAnswerText // Sender fasit
      })
      ...
      // Oppdater kø og poeng (minuspoeng)
...
httpServer.listen(PORT, () => {
  console.log(`Gjett bildet Game Server on port ${PORT}, CORS: ${CORS_ORIGINS.join(', ')}`)
})
